	...
	function p(q) {
		var p=g.Toggles=function(c, a) {
			...
			if("boolean"===typeof a&&c.data("toggles"))c.data("toggles").toggle(a);
				...
				for(var k="on drag click width height animate easing type checkbox".split(" "), b= {}
				, l=0;
				l<k.length;
				l++) {
				...
				a=q.extend( {
					...
					, on:!1, animate:250, easing:"swing", checkbox:null, clicker:null, width:0, height:0, type:"compact", event:"toggle"
				}
				, a|| {}
				, b);
				c.data("toggles", g);
				...
				w.css( {
					height: d, width:l, textIndent:n?"":-d/3, lineHeight:d+"px"
				}
				).html(t.on);
				x.css( {
					height: d, width:l, marginLeft:n?"":-b, textIndent:n?"":d/3, lineHeight:d+"px"
				}
				).html(t.off);
				h.css( {
					height: d, width:d, marginLeft:-b
				}
				);
				s.css( {
					width: 2*m-d, marginLeft:n?0:-m+d
				}
				);
